...
def calc_auroc_per_ds_for_violation(
    ...
    for x in range(training_labels.shape[0]):
        ...
        if training_labels.shape[-1] != pred.shape[-1]:    # remove lags
        ...
        # out = []
        # for w in pred: 
        #     masked = w[~np.eye(w.shape[0], dtype=bool)]
        #     reshaped = masked.reshape(w.shape[0], -1)
        #     out.append(reshaped)       
        # pred = np.stack(out)
        
        # same for the labels: 
        # out = []
        # for w in training_labels[x]: 
        #     masked = w[~np.eye(w.shape[0], dtype=bool)]
        #     reshaped = masked.reshape(w.shape[0], -1)
        #     out.append(reshaped)       
        # labs = np.stack(out)
...
def load_model_from_path_and_predict_violations(violation_stack, path, selected_run, X_eval, testing_labels, X_names_eval):
    ...
    # I need to redo this whole pipeline i guess. 
